...
export default class SuperCamera implements ICameraStrategy<Actor> {
  ...
  public action = (
    ...
    let position = target.getWorldPos();
    ...
    const x = Math.min(
      Math.max(engine.halfDrawWidth, focus.x),
      this.border.x - engine.halfDrawWidth
    );
    const y = Math.min(
      Math.max(engine.halfDrawHeight, focus.y),
      this.border.y - engine.halfDrawHeight
    );
